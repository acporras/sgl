...
namespace SGL.Controllers
{
    public static class Uti
    {
        ...
        public static string agregar_trabajadas(ArrayList trabajadas, string link)
        {
            try
            {
                Excel.Application xlApp = new Excel.Application();
                Excel.Workbook WB = xlApp.Workbooks.Open(link);
                Excel._Worksheet WS = WB.Sheets[1];

                int cole = 2; // collumn element
                int rowe = 2; // row element

                while (true)
                {
                    if (WS.Cells[rowe, cole].value2 == null) { break; }
                    else
                    {
                        trabajadas.Add(WS.Cells[rowe, cole].value2);
                        rowe++;
                    }
                }
        ...
        public static ArrayList merge(string[] list1, ArrayList list2)
        {
        ...
        public static void agregar_otros(ArrayList otros, string user)
        {
            int tamaño = otros.Count;
            int con = 0;
            Array[] paquete = new Array[tamaño];

            foreach (string file in otros)
            {
                XmlDocument xmlDoc = new XmlDocument();
                xmlDoc.Load(file);

                string coddoc = xmlDoc.DocumentElement.ChildNodes[0].ChildNodes[0].ChildNodes[0].ChildNodes[0].ChildNodes[00].ChildNodes[0].InnerText;

                coddoc = coddoc.Substring(15, 13);

                string[] factura = new string[] { coddoc };
                paquete[con] = factura;
                con++;
            }

            Excel.Application xlApp = new Excel.Application();
            Excel.Workbook WB = xlApp.Workbooks.Open("C:\\Users\\" + user + "\\Desktop\\Otros.xlsx");
            Excel._Worksheet WS = WB.Sheets[1];
            ...
            foreach (string[] factura in paquete)
            {
                foreach (string element in factura)
                {
                    WS.Cells[rowe, cole] = element;
                    cole++;
                }
                cole = 2;
                rowe++;
        ...
        public static void listado(Array paquete)
        // Imprime las facturas en un Array de Arrays.
        {
            foreach (Array factura in paquete)
            {
                foreach (string fac in factura)
                {
                    Console.Write(fac + " | ");
                }
                Console.WriteLine("");
        ...
        static Boolean si_listar = true;

        public static void listar(string[] factura)
        // Imprime los valores de una factura en un Array.
        {
            if (si_listar == true)
            {
                foreach (string element in factura)
